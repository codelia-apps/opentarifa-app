 ...
 * [scheduleNextRun]): en cuanto detecta precios publicados, dispara la
 * notificación y dejar de encadenar por hoy; el siguiente eslabón ya se
 * programa entonces para la ventana de mañana. Fuera de la ventana no debe
...
class TomorrowPublishedCheckWorker(context: Context, params: WorkerParameters) : CoroutineWorker(context, params) {
    ...
    override suspend fun doWork(): Result {
        ...
        val notified = try {
            checkAndNotifyTomorrowPricesPublished(applicationContext, notificationPreferencesRepository, pvpcRepository, now.toLocalDate())
        } catch (e: Exception) {
            Log.w(LOG_TAG, "$now — error comprobando publicación, se reintentará", e)
            false
        }

        val delay = if (notified) delayUntilNextWindowStart(now) else RetryInterval
        Log.d(LOG_TAG, "$now — intento en ventana, notified=$notified, siguiente en $delay")
